# Full Guide — Claude Code CLI Workflow

> The thorough version. Read [`quick-start.md`](./quick-start.md) first if you haven't installed Claude Code yet.

A practical handover for moving from the Claude VS Code extension to the full Claude Code CLI workflow — plugins, skills, memory, hooks, subagents, and end-to-end feature flow.
...
**Create one:** make `.claude/skills/<name>/SKILL.md` with frontmatter and a clear `description`. Or run `/skill-create` — it analyses recent work and writes the skill file for you.
